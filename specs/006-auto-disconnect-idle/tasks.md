...
- [ ] T001 Review existing services (ConversationStorageService, LiveKitConnectionService) to understand integration points
- [ ] T002 [P] Create feature branch `006-auto-disconnect-idle` from main branch
- [ ] T003 [P] Verify Angular 20.0.0, TypeScript 5.9.2, and RxJS 7.8 are properly configured in package.json
...
- [ ] T004 [P] Create `IdleTimeoutConfig` interface in src/app/models/idle-timeout-config.ts
- [ ] T005 [P] Create `IdleTimerState` interface in src/app/models/idle-timer-state.ts
- [ ] T006 [P] Create `ActivityEvent` discriminated union types in src/app/models/activity-event.ts
- [ ] T007 [P] Create `IdleTimeoutValidationError` interface in src/app/models/idle-timeout-validation-error.ts
- [ ] T008 [P] Add storage utility functions for idle timeout config in src/app/utils/storage.utils.ts
- [ ] T009 Create validation constants (MIN_DURATION, MAX_DURATION, etc.) in src/app/models/idle-timeout-config.ts
...
- [ ] T010 [P] [US1] Create IdleTimeoutService spec skeleton in src/app/services/idle-timeout.service.spec.ts
- [ ] T011 [P] [US1] Write test: "should initialize with inactive timer state" in idle-timeout.service.spec.ts
- [ ] T012 [P] [US1] Write test: "should start timer when connection established" in idle-timeout.service.spec.ts
- [ ] T013 [P] [US1] Write test: "should countdown timer every second" in idle-timeout.service.spec.ts (use jasmine.clock())
- [ ] T014 [P] [US1] Write test: "should reset timer when transcription received" in idle-timeout.service.spec.ts
- [ ] T015 [P] [US1] Write test: "should reset timer when chat message received" in idle-timeout.service.spec.ts
- [ ] T016 [P] [US1] Write test: "should call disconnect when timer reaches zero" in idle-timeout.service.spec.ts
- [ ] T017 [P] [US1] Write test: "should stop timer when manually disconnected" in idle-timeout.service.spec.ts
- [ ] T018 [P] [US1] Write integration test skeleton in tests/integration/idle-timeout-flow.spec.ts
- [ ] T019 [US1] Write integration test: "should auto-disconnect after 2 minutes of inactivity" in tests/integration/idle-timeout-flow.spec.ts
- [ ] T020 [US1] Write integration test: "should maintain connection when activity occurs within timeout" in tests/integration/idle-timeout-flow.spec.ts

**Verify all tests FAIL** ❌ before proceeding to implementation

### Implementation for User Story 1

- [ ] T021 [US1] Create IdleTimeoutService class skeleton with Injectable decorator in src/app/services/idle-timeout.service.ts
- [ ] T022 [US1] Implement private signals (_timerState, _config) and public readonly signals in idle-timeout.service.ts
- [ ] T023 [US1] Inject ConversationStorageService and LiveKitConnectionService dependencies in idle-timeout.service.ts
- [ ] T024 [US1] Implement startTimer() method with RxJS interval for 1-second countdown in idle-timeout.service.ts
- [ ] T025 [US1] Implement stopTimer() method with subscription cleanup in idle-timeout.service.ts
- [ ] T026 [US1] Implement resetTimer() method to restart countdown from configured duration in idle-timeout.service.ts
- [ ] T027 [US1] Create effect to monitor ConversationStorageService.lastMessageAt signal in idle-timeout.service.ts
- [ ] T028 [US1] Implement onTimeout() private method to call LiveKitConnectionService.disconnect() in idle-timeout.service.ts
- [ ] T029 [US1] Implement ngOnDestroy or use takeUntilDestroyed() for cleanup in idle-timeout.service.ts
- [ ] T030 [US1] Load default config (120 second timeout) from constants in idle-timeout.service.ts constructor
- [ ] T031 [US1] Integrate IdleTimeoutService into VoiceChatComponent by injecting service in src/app/components/voice-chat/voice-chat.component.ts
- [ ] T032 [US1] Call idleTimeoutService.startTimer() when voice chat connection established in voice-chat.component.ts
- [ ] T033 [US1] Call idleTimeoutService.stopTimer() when user manually disconnects in voice-chat.component.ts
- [ ] T034 [US1] Add error handling for disconnect failures in idle-timeout.service.ts
...
- [ ] T035 [P] [US2] Create IdleWarningComponent spec skeleton in src/app/components/idle-warning/idle-warning.component.spec.ts
- [ ] T036 [P] [US2] Write test: "should display when timeRemaining <= 30 seconds" in idle-warning.component.spec.ts
- [ ] T037 [P] [US2] Write test: "should hide when timeRemaining > 30 seconds" in idle-warning.component.spec.ts
- [ ] T038 [P] [US2] Write test: "should show formatted countdown timer (MM:SS)" in idle-warning.component.spec.ts
- [ ] T039 [P] [US2] Write test: "should emit dismiss event when dismiss button clicked" in idle-warning.component.spec.ts
- [ ] T040 [P] [US2] Write test: "should have role='alert' and aria-live='assertive'" in idle-warning.component.spec.ts
- [ ] T041 [US2] Add test to IdleTimeoutService spec: "should set isWarning true when timeRemaining <= 30" in idle-timeout.service.spec.ts
- [ ] T042 [US2] Add integration test: "should show warning at 30 seconds and hide after activity" in tests/integration/idle-timeout-flow.spec.ts

**Verify all tests FAIL** ❌ before proceeding to implementation

### Implementation for User Story 2

- [ ] T043 [P] [US2] Create IdleWarningComponent as standalone component in src/app/components/idle-warning/idle-warning.component.ts
- [ ] T044 [P] [US2] Add ChangeDetectionStrategy.OnPush to IdleWarningComponent in idle-warning.component.ts
- [ ] T045 [P] [US2] Define timeRemaining as required input signal using input.required<number>() in idle-warning.component.ts
- [ ] T046 [P] [US2] Define onDismiss as output using output<void>() in idle-warning.component.ts
- [ ] T047 [P] [US2] Create computed signal for formatted time display (MM:SS) in idle-warning.component.ts
- [ ] T048 [P] [US2] Create computed signal shouldShow based on timeRemaining <= 30 in idle-warning.component.ts
- [ ] T049 [US2] Update isWarning state in IdleTimeoutService based on timeRemaining threshold in idle-timeout.service.ts
- [ ] T050 [US2] Create warning banner HTML template with role="alert" aria-live="assertive" in src/app/components/idle-warning/idle-warning.component.html
- [ ] T051 [US2] Add Material icon and dismiss button with aria-label in idle-warning.component.html
- [ ] T052 [US2] Display formatted countdown in warning message in idle-warning.component.html
- [ ] T053 [US2] Style warning banner with warning colors and positioning in src/app/components/idle-warning/idle-warning.component.scss
- [ ] T054 [US2] Ensure WCAG 2.1 AA color contrast in idle-warning.component.scss
- [ ] T055 [US2] Add IdleWarningComponent to VoiceChatComponent template in voice-chat.component.html
- [ ] T056 [US2] Bind timeRemaining signal to IdleWarningComponent input in voice-chat.component.html
- [ ] T057 [US2] Handle onDismiss event to trigger activity and reset timer in voice-chat.component.ts
...
- [ ] T058 [P] [US3] Write test: "should validate timeout duration is >= 30 seconds" in idle-timeout.service.spec.ts
- [ ] T059 [P] [US3] Write test: "should validate timeout duration is <= 3600 seconds" in idle-timeout.service.spec.ts
- [ ] T060 [P] [US3] Write test: "should validate warningThreshold < durationSeconds" in idle-timeout.service.spec.ts
- [ ] T061 [P] [US3] Write test: "should return validation error for invalid config" in idle-timeout.service.spec.ts
- [ ] T062 [P] [US3] Write test: "should save valid config to sessionStorage" in idle-timeout.service.spec.ts
- [ ] T063 [P] [US3] Write test: "should load config from sessionStorage on initialization" in idle-timeout.service.spec.ts
- [ ] T064 [P] [US3] Write test: "should use default config if sessionStorage is empty" in idle-timeout.service.spec.ts
- [ ] T065 [P] [US3] Write test: "should apply custom timeout duration when configured" in idle-timeout.service.spec.ts
- [ ] T066 [US3] Add integration test: "should persist custom timeout across page refresh" in tests/integration/idle-timeout-flow.spec.ts

**Verify all tests FAIL** ❌ before proceeding to implementation

### Implementation for User Story 3

- [ ] T067 [US3] Implement validateConfig() method with business rule validation in idle-timeout.service.ts
- [ ] T068 [US3] Implement saveConfig() method to persist to sessionStorage in idle-timeout.service.ts
- [ ] T069 [US3] Implement loadConfig() method to retrieve from sessionStorage in idle-timeout.service.ts
- [ ] T070 [US3] Update constructor to call loadConfig() on service initialization in idle-timeout.service.ts
- [ ] T071 [US3] Expose updateConfig() public method that validates and persists in idle-timeout.service.ts
- [ ] T072 [US3] Handle validation errors with typed error returns (IdleTimeoutValidationError) in idle-timeout.service.ts
- [ ] T073 [US3] Update startTimer() to use configured durationSeconds from _config signal in idle-timeout.service.ts
- [ ] T074 [US3] Update warning logic to use configured warningThresholdSeconds in idle-timeout.service.ts
...
- [ ] T079 [P] Add comprehensive JSDoc comments to IdleTimeoutService public methods in idle-timeout.service.ts
- [ ] T080 [P] Add JSDoc comments to all model interfaces in src/app/models/
- [ ] T081 [P] Update project README.md with idle timeout feature documentation
...
- [ ] T087 [P] Verify WCAG 2.1 AA compliance for warning colors and contrast ratios
- [ ] T088 Test idle timeout behavior across browsers (Chrome, Firefox, Safari, Edge)
- [ ] T089 [P] Review and refactor code for consistency with Angular 20 best practices
- [ ] T090 [P] Optimize RxJS subscriptions and verify proper cleanup with no memory leaks
- [ ] T091 Update .github/copilot-instructions.md with 006-auto-disconnect-idle feature context
- [ ] T092 Run linter and fix any ESLint violations in new idle timeout files
- [ ] T093 Verify all type definitions match contracts/idle-timeout-types.ts specification
- [ ] T094 Run quickstart.md validation - verify implementation matches documented approach
